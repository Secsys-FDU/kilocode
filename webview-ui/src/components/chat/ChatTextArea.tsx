...
export const ChatTextArea = forwardRef<HTMLTextAreaElement, ChatTextAreaProps>(
	(
		...
		const liveTranscript = useMemo(() => {
			...
			console.log("🎙️ [ChatTextArea] 📝 liveTranscript:", text, "from segments:", segments)
			return text
		}, [segments])
		...
		const updateHighlights = useCallback(() => {
			...
			if (isRecording && previewRanges.length > 0 && recordingStartStateRef.current) {
				...
				for (let i = previewRanges.length - 1; i >= 0; i--) {
					...
					console.log(
						`🎙️ [ChatTextArea] 🎨 Range ${i}: start=${start}, end=${end}, previewText="${previewText}"`,
					)
				...
				console.log(
					"🎙️ [ChatTextArea] ⏭️ Skipping preview highlighting:",
		...
		}, [customModes, ghostText, isRecording, previewRanges])
			...
			<div
				...
				<DynamicTextArea
					...
					className={cn(
						...
						"cursor-text",
						isEditMode ? "pt-1.5 pb-10 px-2" : "py-1.5 px-2",
						// kilocode_change: Add recording state to border styling
						isRecording
							? "border border-vscode-focusBorder outline outline-vscode-focusBorder ring-2 ring-red-500/30"
							: isFocused
								? "border border-vscode-focusBorder outline outline-vscode-focusBorder"
								: isDraggingOver
									? "border-2 border-dashed border-vscode-focusBorder"
